 ...
 * TIENDA ÉLITE + CAZADOR DE ZOMBIES (ANTI-BUCLES)
 * ═══════════════════════════════════════════════════════
 */
...
// 🧟 EL CAZADOR DE ZOMBIES (Evita el bucle de "Reconectando...")
async function checkActiveMatch(profile) {
  const sb = getSupabase();
  try {
    const { data: activeMatch } = await sb.from('matches').select('*').eq('status', 'playing')
      .or(`player_pink.eq.${profile.id},player_blue.eq.${profile.id}`).limit(1).maybeSingle();

    if (activeMatch) {
      // Verificamos si es una partida fantasma (más de 4 minutos de antigüedad)
      const startTime = new Date(activeMatch.match_start_time).getTime();
      const ageMinutes = (Date.now() - startTime) / 1000 / 60;
      
      if (ageMinutes > 4) {
          // ES UN ZOMBIE. Lo matamos silenciosamente y nos quedamos en el dashboard.
          await sb.from('matches').update({status: 'cancelled'}).eq('id', activeMatch.id);
          return false;
      }

      // Si es una partida fresca, reconectamos normal
...
function statRow(label, val, color, glow) {
  return `<div style="display:flex;align-items:center;justify-content:space-between;">
    <div style="display:flex;align-items:center;gap:.4rem;"><span style="width:9px;height:9px;border-radius:50%;background:${color};box-shadow:0 0 5px ${glow};display:inline-block;"></span><span style="font-family:var(--font-mono);font-size:.68rem;color:var(--text-base);">${label}</span></div>
...
function attachEvents($c) {
  ...
  $c.querySelector('#btn-battle')?.addEventListener('click',   () => {
    const profile = getProfile();
    if (Number(profile.wallet_bs) < 30) { showToast(`Necesitas 30 CP para jugar.`, 'warning'); return; }
    setView('matchmaking');
  });
}
...
function openStoreModal() {
  ...
  showModal(`
    ...
    <div id="store-step-2" style="display:none;">
      ...
      <div class="bank-box" style="margin-bottom:15px;">
        <div class="bank-row"><span class="bank-key">Teléfono</span><div style="display:flex;align-items:center;gap:.4rem;"><span class="bank-val">04144708220</span><button class="btn-copy" onclick="window.__CW_copyToClipboard('04144708220',this)">Copiar</button></div></div>
        <div class="bank-row"><span class="bank-key">Banco</span><div style="display:flex;align-items:center;gap:.4rem;"><span class="bank-val">Banco de Venezuela</span><button class="btn-copy" onclick="window.__CW_copyToClipboard('Banco de Venezuela',this)"></button></div></div>
        <div class="bank-row"><span class="bank-key">C.I.</span><div style="display:flex;align-items:center;gap:.4rem;"><span class="bank-val">30522091</span><button class="btn-copy" onclick="window.__CW_copyToClipboard('30522091',this)">Copiar</button></div></div>
      </div>
  ...
  document.querySelectorAll('.pack-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      document.getElementById('sel-bs').innerHTML = `${parseFloat((selectedUSD * rate).toFixed(2)).toLocaleString('es-VE')} Bs<br><span style="font-size:0.7rem; color:#fff; font-family:var(--font-mono); font-weight:normal;">(${selectedUSD.toFixed(2)} USD x ${rate} Bs/$)</span>`;
...
async function submitStorePurchase(usd, cpAmount, rate) {
  ...
  try {
    ...
    hideModal(); showToast(`Compra de ${cpAmount} CP enviada. Esperando verificación.`, 'warning', 6000);
  } catch (error) { $err.textContent = error.message; } 
  finally { $btn.disabled = false; $btn.textContent = 'COMPRAR CP'; $btn.style.opacity = '1'; }
}
...
function openWithdrawModal() {
  ...
  showModal(`
    ...
    <div class="field-group"><label class="field-label" for="wd-amount">Cantidad a canjear</label><input id="wd-amount" type="number" min="200" step="1" class="input-field" placeholder="Mínimo: 200 CP" inputmode="decimal" style="border-color:#ff00ff;" /><p style="font-family:var(--font-mono); font-size:0.55rem; color:#ff4444; margin-top:5px;">* El retiro mínimo es de 200 CP</p></div>
    ...
    <div class="field-group"><label class="field-label" for="wd-phone">Teléfono</label><input id="wd-phone" type="tel" class="input-field" placeholder="04XX-XXXXXXX" inputmode="tel" /></div>
    <div class="field-group"><label class="field-label" for="wd-ci">Cédula</label><input id="wd-ci" type="text" class="input-field" placeholder="V-XXXXXXXX" inputmode="numeric" /></div>
  ...
  $amount?.addEventListener('input', () => {
    ...
    if (!isNaN(cp) && cp > 0) { $bsRes.textContent = parseFloat(((cp/100) * rate).toFixed(2)).toLocaleString('es-VE') + ' Bs'; $calc.style.display = 'block'; } else { $calc.style.display = 'none'; }
  });
  ...
  function renderBanks(filterText = '') {
      ...
      BANKS.filter(b => b.toLowerCase().includes(filterText.toLowerCase())).forEach(b => {
          const div = document.createElement('div'); div.className = 'bank-option'; div.textContent = b;
          div.onclick = () => { $bankInput.value = b; $bankDropdown.style.display = 'none'; }; $bankDropdown.appendChild(div);
      });
  }
  ...
  document.addEventListener('click', (e) => { if (document.querySelector('.custom-select-wrapper') && $bankDropdown && !document.querySelector('.custom-select-wrapper').contains(e.target)) $bankDropdown.style.display = 'none'; });
  ...
  document.getElementById('btn-wd-submit')?.addEventListener('click', async () => {
    ...
    if (!bank || !phone || !ci) { $err.textContent = 'Llena todos los datos.'; return; }
    ...
    try {
      ...
      hideModal(); showToast(`Solicitud de canje enviada.`, 'info', 6000); await reloadProfile(); setView('dashboard');
    } catch (error) { $err.innerHTML = `<strong>Error:</strong> ${error.message}`; } 
    finally { $btn.disabled = false; $btn.textContent = 'SOLICITAR CANJE'; $btn.style.opacity = '1'; }
  });
}
